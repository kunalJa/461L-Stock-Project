...
export default function StockPage({ data }) {
  ...
  stock.historical[0].dates.forEach((key, i) => {
    ...
    if (i >= stock.historical[0].dates.length - 7) {
      ...
      personalRating +=
        stock.historical[0].prices[i] - stock.historical[0].prices[i - 1]
    }
  ...
  return (
    <>
      ...
      <div
        ...
        className="d-flex flex-direction-row center mb-5"
      >
        <div className="">
      ...
      <h2 style={{ marginLeft: 45, marginTop: 45, fontWeight: "bold" }}>
        Recent Changes (2020)
      </h2>
      <div className="bg-white">
        <LineChart data={weeksData} min={null} curve={false} />
      </div>
      <h2 style={{ marginLeft: 45, marginTop: 50, fontWeight: "bold" }}>
        Weekly High: {high}
      </h2>
      <h2 style={{ marginLeft: 45, marginTop: 50, fontWeight: "bold" }}>
        Weekly Low: {low}
      ...
      <div className="card-deck" style={{ paddingLeft: 15, paddingRight: 15 }}>
        <CardGrid
          items={stock.news.data.slice(1, 3).map(article => {
            return {
              title: article.title,
